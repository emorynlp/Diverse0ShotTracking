...
from dextrous.get_sgd_results import metrics
...
def main(_):
  ...
  all_metric_aggregate, _ = get_metrics(dataset_ref, dataset_hyp, eval_services,
                                        in_domain_services)
  tf.logging.info("Dialog metrics: %s", json.dumps(all_metric_aggregate[ALL_SERVICES], indent=2))
...
>> cd /local/scratch/sfillwo/dextrous/dextrous

>> python -m dextrous.get_sgd_results.evaluate \
    --dstc8_data_dir data/original_sgd \
    --eval_set test \
    --prediction_dir ex/LlamaTracker/MajesticZeb/0/formatted_results/unseen \
    --output_metric_file ex/LlamaTracker/MajesticZeb/0/test_metrics.json
  """
